  ...
  return (
    <ResponsiveContainer width="100%" height={230}>
      <AreaChart data={data} margin={{ left: -10, right: 10, top: 10, bottom: 0 }}>
        ...
        <XAxis dataKey="label" tick={{ fontSize: 11, fill: "#6B7280" }} axisLine={false} tickLine={false} dy={6}
          interval={range === "30D" ? 4 : 0} />
        <YAxis tick={{ fontSize: 11, fill: "#6B7280" }} axisLine={false} tickLine={false}
          tickFormatter={(v) => `$${v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v}`} width={40} />
        <Tooltip
          contentStyle={{ background: "#1A1A1A", border: "1px solid #2D2D2D", borderRadius: "10px", fontSize: "13px", color: "#F1F5F9" }}
          labelStyle={{ color: "#9CA3AF", marginBottom: "4px" }}
          formatter={(v: any) => [`$${Number(v).toLocaleString("en-CA", { minimumFractionDigits: 2 })}`, "Spent"]}
        />
        <Area type="monotone" dataKey="total" stroke="#3B82F6" strokeWidth={2} fill="url(#areaGrad)"
          dot={{ fill: "#3B82F6", r: 3, strokeWidth: 0 }} activeDot={{ r: 5, fill: "#60A5FA" }} />
      </AreaChart>
    </ResponsiveContainer>
  );
}